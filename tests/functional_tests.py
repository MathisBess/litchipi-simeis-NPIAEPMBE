...
class TestAPI:
    ...
    def assign_crew_to_ship(self, sta, ship_id, crew_id, mod_id):
        if mod_id == "pilot":
            return self.post(
                f"/station/{sta}/crew/assign/{crew_id}/ship/{ship_id}/pilot"
            )
        else:
            return self.post(
...
def run_scenario_1_economy(ip, port):
    ...
    assert status_after_ship["money"] < initial_money, (
        "L'argent doit avoir diminué après l'achat du vaisseau"
    )
    ...
    assert status_after_mod["money"] < money_before_mod, (
        "L'argent doit encore avoir diminué après l'achat du module"
    )
    print("Scénario 1 OK !\n")
...
if __name__ == "__main__":
    ...
    server_process = subprocess.Popen(
        ["cargo", "run", "-p", "simeis-server", "--features", "heavy-testing"]
    )
