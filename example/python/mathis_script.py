...
class MathisAPI:
    ...
    def authenticate(self, pseudo):
        ...
        if res.get("error") == "ok":
            ...
            print(
                f"Authentification réussie pour le joueur {pseudo} (ID: {self.player_id})"
            )
            return True
...
if __name__ == "__main__":
    ...
    if api.authenticate(pseudo):
        ...
        if ships_available:
            ...
            print(
                f"Succès ! Le premier vaisseau en vente est le modèle '{first_ship['id']}'"
            )
            print(
                f"Il coûte {first_ship['price']} crédits et possède {first_ship['cargo_capacity']} places de soute."
            )
        else:
